...
Current state (2026-06-30 20:22 +05:30, Codex/GPT-5): the `medgemma_1_5_4b` 200-case Workbench run is complete, promoted, synced to GCS, and downloaded locally under `results/medgemma_1_5_4b_medical_full_200_cases/`. The attempted `llava_med_mistral_7b` vLLM run failed because vLLM 0.23.0 / Transformers did not recognize `model_type=llava_mistral`. The first SGLang attempt stopped early because SGLang installed CUDA-13 PyTorch while the existing CUDA-12.9 torchvision from the vLLM path remained installed; the CUDA vision stack is now fixed on Workbench. The second SGLang attempt imported the LLaVA-Mistral SGLang model class but the server still exited during startup because the child process' Transformers `AutoConfig` did not know `llava_mistral`. Do not pivot to OctoMed without asking the user. The isolated LLaVA copy at `notebooks/RadLE_Medical_Workbench_LLaVA_SGLang_Runtime.ipynb` now removes vLLM in the SGLang path, installs `sglang[all]`, force-reinstalls the matching official PyTorch CUDA-13 trio (`torch==2.11.0`, `torchvision==0.26.0`, `torchaudio==2.11.0` from the `cu130` index), probes SGLang `LlavaMistralForCausalLM`, and writes a child-process `sitecustomize.py` shim that registers Transformers `AutoConfig` key `llava_mistral` as a subclass of `LlavaConfig` before the SGLang server starts. It still selects `llava_med_mistral_7b` and keeps `RUN_LABEL_BASE = "medical_full_200_cases"`, model-scoped `RUN_ID`, `TEST_LIMIT = None`, `RESUME = True`, `EXPECTED_FULL_CASES = 200`, `EXPECTED_FULL_IMAGES = 263`, and `MAX_OUTPUT_TOKENS = 2048`. Next: pull latest `main` on Workbench, restart the kernel, reload/open the LLaVA SGLang notebook copy from disk, rerun cells top-to-bottom, and confirm the config cell prints `llava_mistral config shim OK` before starting the server.
...
- The second SGLang server attempt failed because the launcher still calls Transformers `AutoConfig` before SGLang model dispatch; the LLaVA copy must prepend a runtime `sitecustomize.py` shim that registers `llava_mistral` to `LlavaConfig`.
...
- [x] (2026-06-30 20:22 +05:30, Codex/GPT-5) Patched the isolated LLaVA SGLang notebook to write `/home/jupyter/radle_runtime_shims/sitecustomize.py`, prepend that directory to `PYTHONPATH`, and run a subprocess probe expecting `llava_mistral config shim OK` before server startup.
- [ ] (next Workbench session, user/Codex) Pull latest `main` on Workbench, restart kernel, open/reload `notebooks/RadLE_Medical_Workbench_LLaVA_SGLang_Runtime.ipynb` from disk, run cells top-to-bottom, confirm the `llava_mistral` shim probe prints OK, and audit actual files before promoting or syncing.
