...
**POINTER (2026-07-02, Claude/Opus 4.8):** the notebook-based vLLM/SGLang serving
battle below (LLaVA-Med SGLang abandonment, vLLM pivot, OctoMed/InternVL vLLM
notebook attempts) was superseded on 2026-07-01 by an **Ollama GGUF serving
path** that is now the decided route for ALL open medical VLMs, run via small
standalone scripts instead of per-model notebooks. Current status: LLaVA-Med,
OctoMed-7B, and InternVL3.5-8B are all COMPLETE and PROMOTED via Ollama; next up
are Lingshu-32B and RadFM. The live source of truth is
`Documents/execplan_medical_vlm_ollama_runtime.md` — read that first. This VM
spec section and the dataset/GCS facts below are still accurate and load-bearing;
the notebook serving-stack narrative below this point is HISTORY (kept for the
record of why vLLM/SGLang were abandoned), not the active plan.
...
Validated on Workbench through commit `1d2a84d`: server loads, §6.5 smoke confirmed image ingestion (prompt_tokens 371->978, 256-token completion) BUT the completion ECHOED the prompt instead of answering — `vicuna_v1.1` (USER:/ASSISTANT:) is wrong for this Mistral-Instruct model. Fixes since: commit `db7cf86` switched §4 to write a mistral-instruct chat template JSON (`[INST]`/`[/INST]`) and pass it via `--chat-template`, and hardened §6.5 to also fail on echoed prompt phrases. First template attempt failed with `KeyError: 'system'` because SGLang's `_load_json_chat_template` wants the simple schema (`system`/`user`/`assistant`), not the Conversation-dataclass schema; commit `6cb3fc9` corrected the keys (system="", user="[INST]", assistant="[/INST]", sep_style="LLAMA2", image_token="<image>\n"). HEAD is now `6cb3fc9`.

VERDICT (2026-07-01, Claude/Opus 4.8, agreed with user): the LLaVA-Med **SGLang path is ABANDONED** — it hit the agreed stopping rule. On Workbench HEAD `080ec65` the server came up and every §4 shim applied (`CLIPVisionConfig/MistralConfig positional-arg shim applied`, `LlamaTokenizer.image_processor (CLIP) shim applied`, `llava_mistral config shim OK: LlavaMistralConfig llava_mistral pad_token_id= 2 vision_feature_layer= -2`), the custom mistral chat template loaded with NO parse error (`Loading chat template from argument: .../llava_med_mistral_chat_template.json`), and §6.5 confirmed the image is SPLICED IN (prompt_tokens 371->949 = exactly 576 CLIP visual tokens; the short-prompt probe went 24->600). BUT the model never produces a diagnosis. A non-committed 4-way diagnostic probe against the live server returned: `[A full/greedy]` 1-tok EOS empty; `[B full/temp0.7]` 1-tok EOS empty; `[C short/greedy]` 1-tok EOS empty; `[D short/temp0.7]` 4 toks = `'image> '`. Across TWO templates the model only ever echoes the prompt (vicuna echoed the text radiologist instructions; mistral echoes the literal `<image>` placeholder) and NEVER engages the image. Root cause: the visual features are not driving generation. llava-med ships no `preprocessor_config.json`, so the CLIP image_processor was hand-attached AND the HF-projector config fields (`vision_feature_layer`, `vision_feature_select_strategy`, `projector_hidden_act`, `multimodal_projector_bias`) were GUESSED in the shim; a chat-template knob cannot repair a dead feature/projector path. Per the HARD RULE (no more SGLang-internal shims), we stop and pivot. The 4-way probe and the smoke output were NOT committed (throwaway scratch cell); the SGLang notebook is unchanged at `080ec65`.

PIVOT — ACTIVE NEXT TASK FOR CODEX (decided 2026-07-01; user explicitly chose "vLLM + corrected config" over Ollama / image_token probe / dropping LLaVA): serve LLaVA-Med on **vLLM's mature LLaVA pipeline using an HF-format checkpoint**, so vLLM reads the REAL projector weights from the checkpoint instead of us guessing config fields. KEY FACT from research (sources: HF model `chaoyinshe/llava-med-v1.5-mistral-7b-hf`, transformers `convert_llava_weights_to_hf.py`, vLLM issues #10129/#11449): `microsoft/llava-med-v1.5-mistral-7b` ships in the ORIGINAL LLaVA weight layout (`model.mm_projector.*`, `model.vision_tower.*`, a flat MistralConfig plus `mm_*` fields), NOT the HF `LlavaForConditionalGeneration` layout, so a `config.json` relabel / `--hf-overrides` alone will NOT satisfy vLLM's weight loader — the weight NAMES differ. Two routes:

- **PREFERRED:** a community pre-converted HF checkpoint already exists — `chaoyinshe/llava-med-v1.5-mistral-7b-hf` (architectures = `LlavaForConditionalGeneration`). Point vLLM at it directly; no conversion needed.
- **FALLBACK:** convert the original checkpoint locally with transformers' `convert_llava_weights_to_hf.py` (build HF `LlavaConfig` = MistralConfig text_config + CLIP ViT-L/14-336 vision_config + `image_token_index`; remap `model.mm_projector.0/2` -> `multi_modal_projector.linear_1/2`, `model.vision_tower.vision_tower.vision_model.*` -> `vision_tower.vision_model.*`, language `model.*` -> `language_model.model.*`; resize embeddings for the added `<image>` token), then serve the local dir.

AUTHORITATIVE RUNBOOK: the step-by-step execution plan for this pivot lives in the child ExecPlan **`Documents/execplan_llava_vllm_runtime.md`** (Codex-owned, kept current). It already carries the full handoff: the live §6.5 probe evidence, the Milestone 1 checkpoint-verification gate, the registry edit (`llava_med_mistral_7b` -> model_id `chaoyinshe/llava-med-v1.5-mistral-7b-hf`, `preferred_engine="vllm"`, keep `name` for run-ID stability), the new `notebooks/RadLE_Medical_Workbench_LLaVA_vLLM_Runtime.ipynb` build from the MedGemma vLLM pattern, the real-diagnosis §6.5 smoke gate, and the audit/repair/promote/export/sync guardrails. Summary next action: (1) hf-cli verify the HF checkpoint; (2) patch only the LLaVA registry entry; (3) build the vLLM notebook with NO SGLang/chat-template/shim carryover; (4) keep run ID `llava_med_mistral_7b_medical_full_200_cases`, 200 cases / 263 images, `TEST_LIMIT=None`, `RESUME=True`, `MAX_OUTPUT_TOKENS=2048`. Standing guardrails: verify full notebook cell structure before committing (a NotebookEdit corrupted the SGLang notebook once, `cc7c4bf` -> `100e8d6`); don't trust logs, audit files; never promote/sync a void/unaudited run; do NOT touch OctoMed or InternVL. Ollama GGUF+mmproj remains a documented Plan B only if vLLM also fails.
...
- [x] (2026-07-01, user on Workbench and Claude/Opus 4.8) Validated `1d2a84d`: server loaded, §6.5 smoke confirmed image ingestion (prompt_tokens 371->978, non-empty 256-token completion) — but the model ECHOED the prompt (recited the radiologist instructions) because `vicuna_v1.1` is the wrong conversation format for a Mistral-Instruct model.
- [x] (2026-07-01, Claude/Opus 4.8) Switched §4 to a mistral-instruct chat template JSON via `--chat-template`, and hardened §6.5 to fail on echoed prompt phrases (commit `db7cf86`). First attempt failed `KeyError: 'system'` — SGLang's JSON loader wants the simple `system`/`user`/`assistant` schema; corrected keys in commit `6cb3fc9` (sep_style LLAMA2, image_token "<image>\n").
- [x] (2026-07-01, user on Workbench + Claude/Opus 4.8) Validated `080ec65` (mistral template, `6cb3fc9` lineage): server + all shims OK, template loaded with no parse error, §6.5 confirmed image splice (prompt 371->949 = 576 visual tokens). FAILED on output — model returns 1-token EOS / empty. A 4-way non-committed diagnostic probe (full/short x greedy/temp0.7) showed empty everywhere except `[D short/temp0.7]` = `'image> '`. Across two templates the model never engages the image; visual features are not driving generation (guessed projector config + hand-attached CLIP processor). Declared SGLang LLaVA path dead per stopping rule.
- [x] (2026-07-01, Claude/Opus 4.8) Researched the vLLM pivot. Found a community pre-converted HF checkpoint `chaoyinshe/llava-med-v1.5-mistral-7b-hf` (architectures=`LlavaForConditionalGeneration`) and the transformers `convert_llava_weights_to_hf.py` fallback. Established that the original `microsoft/llava-med-v1.5-mistral-7b` weight LAYOUT (not just config) differs from HF Llava, so vLLM needs an HF-format checkpoint, not a config relabel.
- [x] (2026-07-01, user) Chose the vLLM + corrected-config pivot (over Ollama / image_token probe / dropping LLaVA).
- [ ] (NEXT, Codex) Execute the vLLM pivot per `Current State` PIVOT block: (1) hf-cli verify `chaoyinshe/llava-med-v1.5-mistral-7b-hf` config/architectures/preprocessor; (2) build `notebooks/RadLE_Medical_Workbench_LLaVA_vLLM_Runtime.ipynb` cloned from the MedGemma vLLM path (vllm, float16, JSON `--limit-mm-per-prompt {"image": 5}`), model_id = the HF repo, with a §6.5 real-diagnosis smoke gate and no SGLang shim machinery; (3) keep run ID `llava_med_mistral_7b_medical_full_200_cases`, 200 cases / 263 images, `TEST_LIMIT=None`, `RESUME=True`, `MAX_OUTPUT_TOKENS=2048`. Validate notebook cell structure before committing; push to `main` for Workbench `git pull`.
...
- Observation: A green SGLang server + correct image-token COUNT does not prove the visual FEATURES reach the language model. LLaVA-Med spliced exactly 576 CLIP tokens (prompt 371->949) yet produced only 1-token EOS / empty across greedy, sampled, short, and long prompts, with sampling once emitting the literal placeholder `'image> '`. Under two different chat templates the model only ever echoed the prompt and never described the image.
  Evidence: 4-way probe `[A full/greedy]=''`, `[B full/temp0.7]=''`, `[C short/greedy]=''`, `[D short/temp0.7]='image> '`. The likely cause is the guessed HF-projector config fields + hand-attached CLIP image_processor (llava-med ships no `preprocessor_config.json`), which can produce garbage image embeddings the LM ignores. Lesson: the §6.5 smoke must assert a REAL diagnosis, not just a prompt-token jump.
...
- Decision: Abandon the LLaVA-Med SGLang serving path and pivot to vLLM with an HF-format checkpoint (`chaoyinshe/llava-med-v1.5-mistral-7b-hf`, or a local `convert_llava_weights_to_hf.py` conversion as fallback).
  Rationale: The SGLang path required hand-attaching a CLIP image_processor and GUESSING HF-projector config fields (llava-med ships no `preprocessor_config.json`). §6.5 proved the image is spliced (576 tokens) but the model never engages it (1-token EOS / `'image> '` echo across 4 configs and 2 templates), so the visual features/projector are dead — a chat-template knob cannot fix it. This hit the agreed stopping rule (no more SGLang-internal shims). vLLM's mature LLaVA pipeline reads the real projector weights from the checkpoint, removing the guessing; the original vLLM failure was only `model_type=llava_mistral` unrecognized, which an HF-format checkpoint resolves. The user explicitly chose vLLM over Ollama, the cheap image_token probe, and dropping LLaVA.
  Date/Author: 2026-07-01, user and Claude/Opus 4.8
