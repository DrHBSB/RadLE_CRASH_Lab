...
Monitoring uses UTC timestamps, per-call elapsed time, readable retry/review reasons and a count of collected answers, complete cases, running calls and queued work. Long calls get a 30-second waiting update. Structured stream errors include token limits and the archive path, without printing clinical response text or credentials. Code versions are recorded separately from request/input identity so logging-only upgrades can reuse the journal without changing saved outcomes.
