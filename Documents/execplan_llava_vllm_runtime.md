...
- [ ] (next, user via Workbench Jupyter) Pull, restart kernel, rerun Section 3 (reload parser), then run Section 8.5 against the existing CSV and paste its output. Do NOT clear the CSV yet. The 8.5 counts decide: if reparse-still-fails is high, the parser needs work; if STALE is high, the run must force-re-run those cells.
