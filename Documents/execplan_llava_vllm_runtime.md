...
Current state (2026-07-01 12:18 +05:30, Codex/GPT-5): branch `codex/llava-vllm-runtime` reached a fresh GUI run at coordinator head `409ce2b`: Section 4 showed the correct `chaoyinshe/llava-med-v1.5-mistral-7b-hf` vLLM mapping and `Request extra body: {'min_tokens': 16}`, Section 6 `/v1/models` returned the correct model, and Section 6.5 still failed on case `156` with blank visible output, `finish_reason=length`, `prompt_tokens=3256`, and `completion_tokens=2048`. No Workbench/Jupyter runtime success has been claimed. This commit changes the LLaVA-only request body to expose special tokens and block EOS/PAD string candidates, and the smoke cell now prints the unstripped raw response, a `repr(...)` preview, and raw character length. Next: pull this commit or newer, restart/clear if needed, run only through Section 6.5, and stop before Section 7.
...
The first static vLLM implementation slice was pushed, and the branch/mapping correction was proven in the GUI at commit `0b90a60`. The helper-level vLLM optional rotary FlashAttention fallback patch moved the run past server startup: GUI evidence now repeatedly shows `/v1/models` returning `chaoyinshe/llava-med-v1.5-mistral-7b-hf`. The next unproven gate is still Section 6.5 runtime generation: immediate-EOS fixes moved the failure from `finish_reason=stop` with one token to `finish_reason=length` with 2048 completion tokens, but the visible response is still blank after stripping. This commit exposes the raw decoded payload and likely special-token loop candidates in the next GUI run. No Workbench/Jupyter smoke or full run has been proven yet. The main reusable lesson is project-specific here: a smoke gate must prove a real benchmark-style JSON diagnosis, not just server readiness, image-token insertion, or completion-token growth, and remote notebook bootstraps must make both the imported repo revision and stale-cell risk visible.
