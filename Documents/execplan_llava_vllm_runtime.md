...
- [ ] (next, user via Workbench Jupyter) Pull, reload notebook FROM DISK, restart kernel. Run Section 6 → 6.5 smoke ONLY first and paste the raw response — this shows the natural image output format with min_tokens-only (no newline-blocking). If it parses (clean or VQA), delete `.../raw/` and run Section 7 full + 8.5. If it's prose the parser can't handle, paste it and we enhance extract_json_safely before the full run.
