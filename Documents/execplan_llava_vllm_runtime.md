...
- [ ] (next, user on VM terminal) Pull, then run `python scripts/llava_med_hf_probe.py` from the repo root. Paste the two CASE outputs + the VERDICT block. That decides: fix vLLM serving vs. abandon this checkpoint.
