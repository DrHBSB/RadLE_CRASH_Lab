...
- [ ] (next) Decide integration + how to elicit/extract a diagnosis (not just a description) from LLaVA-Med while preserving cross-model prompt parity. Options: wire ollama as a serving engine (START_SERVER=False, BASE_URL=:11434/v1, model=z-uo tag, extra_body=None) and lean on the audit -> LLM-judge repair path to extract diagnoses; and/or check whether the description-not-diagnosis behavior holds across more cases before committing to a full run.
