...
- [ ] (next, user via Workbench Jupyter) Pull, restart kernel. CLEAR the stale run first: delete `.../Runs/llava_med_mistral_7b_medical_full_200_cases/raw/` (results.csv + backups) so no bad_words/band-aid rows survive resume. Then run Section 6 → 6.5 (smoke must pass with clean output, no immediate-EOS) → 7 (full run) → 8.5 (inspect). If image cases regress to completion_tokens=1 immediate-EOS, add back ONLY min_tokens=16 and report.
