...
- [x] (2026-07-01, user via Workbench Jupyter) Smoke PASSED: case 156 -> `1. Pulmonary tuberculosis: 3 (moderate confidence)`, finish_reason=stop, parsed diagnosis="Pulmonary tuberculosis" likert=3. EOS loop fixed, VQA parser works. User then ran the full benchmark (resume over an existing 200-row CSV from prior bad_words runs); the scorer view showed `PARSE_FAILED` for all displayed cases. Open question: stale rows kept by resume vs. genuinely unparseable fresh output (single-image cases may use a different format than the 5-image smoke).
- [x] (2026-07-01, Claude/Opus 4.8) Added non-writing Section 8.5 raw-output inspection cell: shows Raw_Response next to stored and freshly re-parsed diagnosis/likert, plus whole-file counts (stored-fail, reparse-still-fails, stored-fail-but-reparse-now-succeeds = STALE). This distinguishes a parser-regex problem from stale resume rows before any further change.
- [x] (2026-07-01, user via Workbench Jupyter) Ran Section 8.5. NOT stale — fresh re-parse also fails. Raw outputs revealed three modes: (1) prose after the diagnosis (`1.  Pulmonary tuberculosis is a bacterial infection...`) — min_tokens=16 forcing continuation past the diagnosis; (2) `1. 1. 1. 1. 1.` collapse (cases 5,9,10,11,13) — logit_bias blocking newlines on list answers; (3) clean `1. X: 3 (moderate confidence)` (case 16 only). Decisive: probe B already produced perfect JSON `{"diagnosis": "I don't know", "likert_score": null}` with the real RadLE prompt and NO extra_body. The leftover band-aids are corrupting the native output.
- [x] (2026-07-01, Claude/Opus 4.8) Removed the entire LLaVA `request_extra_body` (min_tokens, skip_special_tokens, spaces_between_special_tokens, logit_bias). Updated notebook EXPECTED_REQUEST_EXTRA_BODY to None. Kept the VQA parser fallback as a harmless safety net. Rationale: those guards were band-aids for the bad_words EOS bug (already removed) and were actively degrading output; the model follows the JSON instruction natively.
- [x] (2026-07-01, user via Workbench Jupyter) Clean run with extra_body=None regressed exactly as predicted: probes A/B/C all clean (no loop), but the IMAGE smoke returned completion_tokens=1, empty raw, finish_reason=stop, and the full 200-run produced 200 empty `parse_failed_empty_raw` rows (1-2 completion tokens each). Confirms this checkpoint EOS-terminates immediately on multimodal input; min_tokens is required for image cases (text-only is fine without it).
- [x] (2026-07-01, Claude/Opus 4.8) Restored `request_extra_body={"min_tokens": 16}` — the ONE principled guard — and nothing else (no logit_bias, no bad_words, no skip_special_tokens). Updated notebook EXPECTED_REQUEST_EXTRA_BODY to match.
- [x] (2026-07-01, user via Workbench Jupyter) min_tokens=16 ALONE floods newlines (2047 `\n`, finish_reason=length). So the earlier real-diagnosis output required min_tokens AND newline-suppression together. Image input has TWO degenerate attractors (first-token EOS; newline flood); text input is clean with no constraints. Combined with the near-constant "Pulmonary tuberculosis" outputs, this strongly suggests weak/broken image conditioning in the HF conversion — the exact risk the original handoff flagged. STOPPED stacking constraints.
- [x] (2026-07-01, Claude/Opus 4.8) User deferred the direction decision. Chose: investigate the checkpoint with a DECISIVE test before any more serving hacks or full runs. Added `scripts/llava_med_hf_probe.py` — loads the checkpoint via native `LlavaForConditionalGeneration` + its own processor/chat template, greedy decode, NO constraints, on two visually different real cases (1 and 8), pinned to GPU1 so the vLLM server on GPU0 is untouched. If outputs are real and differ per image -> checkpoint OK, vLLM template is the bug. If empty/newline/identical -> conversion broken, switch model/source (ollama/GGUF or original checkpoint). User is open to ollama.
- [x] (2026-07-01, user on VM terminal) DEFINITIVE: native transformers + the checkpoint's OWN chat template. Natural greedy -> immediate `</s>` (empty) on both cases. Forced (min_new_tokens=64) -> 256 pure newlines, 0 non-whitespace, BYTE-IDENTICAL for case 1 and case 8. Two different medical images produce the exact same newline flood. `chaoyinshe/llava-med-v1.5-mistral-7b-hf` does NOT do image-conditioned generation; the image contributes zero signal. Not a vLLM/template/logit_bias problem — the weights. This also retroactively invalidates the earlier vLLM "Pulmonary tuberculosis" outputs: those were the newline-suppression logit_bias clawing into the model's TEXT prior, not image reading. The whole vLLM+HF-checkpoint route is dead.
- [x] (2026-07-01, Claude/Opus 4.8) PIVOT to Ollama GGUF. Found `z-uo/llava-med-v1.5-mistral-7b_q8_0` on the Ollama library (OpenAI-compatible endpoint at :11434/v1, drops into the benchmark). Added `scripts/ollama_llava_med_probe.py` — same two-image conditioning gate using the benchmark's real base64 payload — because a GGUF built from the broken HF weights could inherit the defect. Do NOT wire Ollama into the benchmark until the probe shows image-varying output.
- [x] (2026-07-01, Claude/Opus 4.8 via Claude-in-Chrome on the VM's JupyterLab terminal) Ran the ollama probe. WIN: the GGUF `z-uo/llava-med-v1.5-mistral-7b_q8_0` READS IMAGES. Case 1 -> "coronal chest CT-scan ... lungs, heart" (75 tok); Case 8 -> "coronal chest CT-scan AND an axial chest CT-scan ... two different views" (110 tok). Different, image-specific, coherent; both finish_reason=stop. Image conditioning works via ollama where the HF conversion was dead. NEW ISSUE: the model DESCRIBES the image instead of emitting the RadLE JSON diagnosis, so extract_json_safely=PARSE_FAILED. Classic LLaVA-Med VQA behavior — a prompting/extraction problem, not a broken model.
- [x] (2026-07-01, Claude/Opus 4.8 + user) Characterized 8 cases: model reads images but only ~1/8 commits to a diagnosis (case 78 -> "presence of a pulmonary embolism"); the rest describe modality/anatomy, one empty, and cases 8 & 45 gave byte-identical boilerplate. Never returns JSON. User decision: BUILD FAILSAFE + RUN FULL (keep the prompt identical for manuscript parity; failsafe extracts diagnosis+likert only as a last resort).
- [x] (2026-07-01, Claude/Opus 4.8) Built two pieces: (1) conservative prose-diagnosis fallback in `extract_json_safely` — fires only on committed phrasing ("presence of/consistent with/reveals ..."), rejects modality/anatomy/possibility lists, never fabricates; unit-tested against all 8 real outputs (only case 78 extracts) + positive controls. (2) `scripts/run_llava_med_ollama.py` — runs the standard RadLE benchmark through Ollama's OpenAI endpoint with the SAME prompt/payload, writes the normal run CSV (run label `medical_full_200_cases_ollama`), and prints a diagnosis-rate summary. Registry/notebook untouched (dead vLLM path); the runner uses an inline model config.
- [ ] (next, user on VM terminal) Optional shakedown `python scripts/run_llava_med_ollama.py --limit=5`, then the full `python scripts/run_llava_med_ollama.py`. Paste the diagnosis-rate summary. Then audit -> repair -> promote under the usual guardrails. Expect many PARSE_FAILED (no-diagnosis) rows — that is LLaVA-Med's genuine behavior, to be handled by the audit/repair path, not forced.
