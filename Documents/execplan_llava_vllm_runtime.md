...
- [ ] (next, user on VM terminal) Install ollama (`curl -fsSL https://ollama.com/install.sh | sh`), `ollama pull z-uo/llava-med-v1.5-mistral-7b_q8_0`, then `python scripts/ollama_llava_med_probe.py`. Paste the two CASE outputs + VERDICT. If it reads images -> integrate Ollama as a serving engine. If broken too -> LLaVA-Med v1.5-mistral does not survive conversion here; decide whether to try the original checkpoint via the official LLaVA-Med repo or drop LLaVA-Med from the comparison.
