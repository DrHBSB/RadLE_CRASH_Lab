...
PANEL_TEXT = {
    ...
    3: {
        ...
        "subtitle": (
            "A calibrated model is at its most accurate when it claims to be perfectly confident (green trend). \"Non-self-aware\" "
            "models exhibit consistently low, noisy accuracy regardless of what they claim (grey), while dangerously uncalibrated "
            "models actually suffer a drop in accuracy when they claim to be absolutely certain (red)."
        ),
...
CSS = f"""
    ...
    .bar-label-white {{ font-size: {css_px("bar-label-white")}; font-family: {MONO}; font-weight: 700; fill: {SURFACE}; }}
        ...
        for _, value, color in segments:
            ...
            if value:
                ...
                if inside_bar_label_fits(value, w):
                    parts.append(text(x + w / 2, y + BAR_LABEL_DY, value, "bar-label", anchor="middle"))
            x += w
...
def panel5(data: list[dict[str, object]]) -> str:
    ...
    for i, row in enumerate(rows):
        ...
        if inside_bar_label_fits(correct_label, correct_w):
            parts.append(text(chart_x + correct_w / 2, y + BAR_LABEL_DY, correct_label, "bar-label", anchor="middle"))
        elif correct_w > 0:
        ...
        if inside_bar_label_fits(wrong_label, wrong_w):
            parts.append(text(chart_x + correct_w + wrong_w / 2, y + BAR_LABEL_DY, wrong_label, "bar-label", anchor="middle"))
        elif wrong_w > 0:
