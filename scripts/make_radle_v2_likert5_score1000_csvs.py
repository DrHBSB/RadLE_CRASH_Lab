...
STATUS_DESCRIPTIONS = {
    "abstention_idk_reward": "Diagnosis is exactly `I don't know`; raw score is +1.",
    "abstention_idk_typo_reward": "Diagnosis is the observed typo `Idon't know`; raw score is +1.",
    "invalid_likert_zero": "Likert is absent or outside 0..4 after non-IDK/non-failure handling; raw score is 0.",
...
HUMAN_GROUPS = [
    ("Board-certified radiologists", BOARD_TIERS),
    ("Radiology trainees", TRAINEE_TIERS),
]
...
GROUP_SUMMARY_COLUMN_MANUAL = [
    ("rank", "Overall rank after sorting `final_score1000` descending; 1 is highest."),
    ...
    ("n_readers", "Number of independent readers represented: 1 for each model, 6 for each human comparator."),
...
GROUP_SUMMARY_COLUMN_MANUAL.extend(
    [
        ...
        (
            "score_vs_all_idk_baseline",
            "`final_score1000 - effective_cases`; positive means the reader beats the all-IDK baseline.",
        ),
        ("idk_rate_pct", "Effective share of exact or typo IDK rows."),
        ("peak_confidence_effective_cases", "Effective count of non-IDK, non-failure answers with Likert 4."),
...
def group_for_human_tier(tier: str) -> tuple[str, int] | None:
    if tier in BOARD_TIERS:
        return ("Board-certified radiologists", 6)
    if tier in TRAINEE_TIERS:
        return ("Radiology trainees", 6)
    return None
...
def validate_human200(source: pd.DataFrame) -> None:
    ...
    expected = {
        "Board-certified radiologists": (1200, 6, 200.0),
        "Radiology trainees": (1200, 6, 200.0),
    }
...
def build_model_summary(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    out = pd.DataFrame(rows).sort_values(
        ["final_score1000", "reader_label"], ascending=[False, True]
    )
    if len(out) != 15:
...
def build_human_summary(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(out) != 2:
        raise GateFailure(f"Expected 2 human summary rows, got {len(out)}")
    out = out[SUMMARY_COLUMNS_WITHOUT_RANK]
    return out
...
def group_summary_manual_lines() -> list[str]:
    lines = [
        ...
        "",
        "`score1000_group_summary.csv` has one row per comparator: 15 non-excluded model arms plus 2 human comparator rows. It is the union of `score1000_model_summary.csv` and `score1000_human_comparator_summary.csv`, sorted by `final_score1000` descending.",
        "",
        ...
        "- Human comparator rows use `score1000_n_readers_in_group = 6`, so each reader-row has weight `1/6`.",
        "- The two human comparator groups each have 1200 raw reader-rows and effective denominator `1200 / 6 = 200`.",
        "",
        "Therefore:",
        "",
        ...
        "- An all-IDK comparator scores `+200` because each effective case scores `+1`.",
        "",
        "## Status Values",
        "",
    ]
    ...
    md_path.write_text(
        "\n".join(
            [
                ...
                "",
                "Score1000 rule: valid Likert 0..4 maps to weight 1..5; correct diagnoses score positive; wrong diagnoses score negative; exact or typo IDK rows score +1; invalid Likert and technical failures score 0.",
                "",
        ...
        "\n".join(
            [
                ...
                *[
                ...
                "- Exact `I don't know` and typo `Idon't know` score `+1`.",
                "- Invalid Likert and technical failure / `PARSE_FAILED` score `0`.",
                "- A 200-case comparator ranges from -1000 to +1000; all-IDK baseline is +200.",
                "",
                "## Weighting Logic",
                "",
                "- Model arms represent one reader over 200 cases: row weight is `1` and effective n is `200`.",
                "- Human comparator groups keep all original reader rows. Each group has 6 readers and 1200 raw rows.",
                "- Human row weight is `1/6`, so each human comparator has effective n `1200 / 6 = 200`.",
    ...
    path.write_text(
        "\n".join(
            [
                ...
                "",
                "Initial generation completed. Run `scripts/audit_radle_v2_likert5_score1000_csvs.py` for the final gate.",
                "",
                ...
                "- Human200 comparator weights sum to 200 each",
...
def main() -> None:
    ...
    print("[PASS] human effective denominators Board-certified radiologists=200 Radiology trainees=200")
