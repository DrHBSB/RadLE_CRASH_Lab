...
from radle_incremental_admission import ValidationError, canonical_text_sha256, sha256_file, validate_base_authority, validate_configs
...
def _cmd_check_config(args: argparse.Namespace) -> int:
    ...
    receipt["config_hashes"] = {
        "roster": canonical_text_sha256(Path(args.roster)),
        "judges": canonical_text_sha256(Path(args.judges)),
        "states": canonical_text_sha256(Path(args.states)),
    }
...
def _cmd_check_base(args: argparse.Namespace) -> int:
    receipt = validate_base_authority(
        authority_manifest=Path(args.base_authority),
        authority_root=Path(args.authority_root),
        roster_path=Path(args.roster),
    )
    print(json.dumps(receipt, indent=2, sort_keys=True))
    print("BASE_RESULT=PASS")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    check_base = subparsers.add_parser("check-base", help="validate frozen production base authorities")
    check_base.add_argument("--base-authority", default="config/radle_v2_base_authority.json")
    check_base.add_argument("--authority-root", required=True)
    check_base.add_argument("--roster", default="config/radle_v2_model_roster.json")
    check_base.set_defaults(func=_cmd_check_base)
    ...
    prepare.add_argument("--blind-map", required=True)
