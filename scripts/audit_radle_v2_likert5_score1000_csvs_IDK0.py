...
def configure_idk_score(value: int) -> None:
    global IDK_SCORE
    if value not in {0, 1}:
        raise AuditFailure(f"Unsupported IDK score {value}; expected 0 or 1")
    IDK_SCORE = value
...
def validate_source_and_scored(frames: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    human_groups = set(human["score1000_group"])
    if human_groups != {"Human Expert Baseline"}:
        raise AuditFailure(f"Unexpected human comparator groups: {sorted(human_groups)}")
    frame = human[human["score1000_group"] == "Human Expert Baseline"]
    if len(frame) != 2400:
        raise AuditFailure(f"Human Expert Baseline: expected 2400 source rows, got {len(frame)}")
    if frame["provider"].nunique() != 12:
        raise AuditFailure(f"Human Expert Baseline: expected 12 readers, got {frame['provider'].nunique()}")
    if not close_enough(frame["score1000_row_weight"].astype(float).sum(), 200.0):
        raise AuditFailure("Human Expert Baseline: effective denominator does not sum to 200")
...
def validate_summaries(frames: dict[str, pd.DataFrame], scored: pd.DataFrame) -> None:
    ...
    expected_order = group_summary.assign(_score2000=pd.to_numeric(group_summary["score2000"], errors="raise")).sort_values(
        ["_score2000", "reader_label"], ascending=[False, True]
    )["reader_label"].tolist()
    actual_order = group_summary["reader_label"].tolist()
    if actual_order != expected_order:
        raise AuditFailure("Group summary rank order is not sorted by score2000 descending")
...
def validate_manifest(out_dir: Path) -> None:
    ...
    if generator != "scripts\\make_radle_v2_likert5_score1000_csvs_IDK0.py":
...
def write_audit_report(out_dir: Path, scored: pd.DataFrame) -> None:
    ...
    human_totals = {
        ...
        for group in ("Human Expert Baseline",)
    }
    report = {
    ...
    md_path.write_text(
        "\n".join(
            [
                ...
                f"- Human Expert Baseline: {human_totals['Human Expert Baseline']}",
                "",
            ]
        ),
        encoding="utf-8",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=[0, 1],
        default=1,
        help="Raw score expected for exact `I don't know` and typo `Idon't know` rows.",
    )
